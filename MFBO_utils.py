...
def initialize_model(X, Y, noise=None, state_dict=None):
...
def direct_link(acqf,path="input_data/"):
    pts_path = path+"opt_points"+"/"+acqf+".pt"
    mdl_path = path+"gp_model"+"/"+acqf+".pt"
    df_pts = torch.load(pts_path)

    X_lofi, Y_lofi, = df_pts[:,:-1], df_pts[:,-1].unsqueeze(-1)

    mll,gp_model = initialize_model(X_lofi, Y_lofi)
    gp_model.load_state_dict(torch.load(mdl_path))

    return mll, gp_model, X_lofi, Y_lofi
